// Electric Brightness - P5.js Sketch
// Star field visualization with curved connecting lines

let img;
let stars = [];
let currentIndex = 0;
let speed = 0.41; // Speed of animation
...
function setup() {
  ...
  let soundLink = createA('/electricBrightnessSound/build', 'see with sounds', '_parent');
...
function windowResized() {
  // Recalculate canvas size and position when window is resized
  const maxWidth = windowWidth * 0.8;
  const maxHeight = windowHeight * 0.8;
  const scaleFactor = Math.min(maxWidth / img.width, maxHeight / img.height);
  const newWidth = img.width * scaleFactor;
