...
let stars = [];
let currentIndex = 0;
let speed = 0.01; // Slower animation speed
let moveSpeed = 0.005; // Movement speed across lines
let cameraMoveSpeed = 0.02; // Speed of the camera moving towards the star
let threshold = 200;
let strokeWeightValue = 3;
let gradientResolution = 0.05;
let bpm = 50; // Lower BPM
...
let stayDuration = 200; // Duration to stay at each star
let stayTimer = 0;
...
function setup() {
    ...
    cameraPosition = createVector(0, 0, 500);
    targetCameraPosition = createVector(0, 0, 500);
}
...
function setupCanvas() {
    ...
    canvas = createCanvas(newWidth, newHeight, WEBGL);
    ...
    for (let y = 0; y < img.height; y++) {
        for (let x = 0; x < img.width; x++) {
            ...
            if (brightnessValue > threshold) {
                let z = random(-200, 200); // Add random z-coordinate
                stars.push({ x: x - img.width / 2, y: y - img.height / 2, z: z, brightness: brightnessValue });
            }
        }
    }
...
function setupAudioNodes() {
    sampler = new Tone.Sampler({
        ...
        attack: 0.1, decay: 0.2, sustain: 0.9, release: 0.2, baseUrl: "/starsounds3d/samples/",
...
function scheduleAudio() {
    ...
    stars.forEach((star, index) => {
        Tone.Transport.schedule(() => playBassSynth(star), index * (random() * 2 + 2) + 20); // Longer delay between stars
    });
...
function drawGradientLine(starIndex, energy) {
    let startColor = color(mouseX, 55, 0, 100);
    let endColor = color(255, 255, 0, 100);
...
function draw() {
    ...
    rotationAngle += 0.05; // Increase rotation angle for continuous rotation
}
...
function drawStars() {
    for (let star of stars) {
        ...
        stroke(255);
        let starSize = map(p5.Vector.dist(createVector(star.x, star.y, star.z), cameraPosition), 0, 500, 10, 1);
        starSize = constrain(starSize, 1, 10); // Constrain the size to a reasonable range
...
function updateCamera() {
    ...
    if (stayTimer < stayDuration) {
        ...
        if (lerpT >= 1) {
            ...
            currentStar = stars[targetStarIndex];
            nextStar = stars[(targetStarIndex + 1) % stars.length];
            stayTimer = 0; // Reset the stay timer
        }

        let target = p5.Vector.lerp(createVector(currentStar.x, currentStar.y, currentStar.z), createVector(nextStar.x, nextStar.y, nextStar.z), lerpT);
        targetCameraPosition = p5.Vector.lerp(cameraPosition, target, cameraMoveSpeed);

        let lookAtX = nextStar.x;
        let lookAtY = nextStar.y;
        let lookAtZ = nextStar.z;

        camera(targetCameraPosition.x, targetCameraPosition.y, targetCameraPosition.z, lookAtX, lookAtY, lookAtZ, 0, 1, 0); // Look at next star
...
function startVolumeModulation() {
    const now = Tone.now();
    gainNode.gain.setValueAtTime(MIN_VOLUME, now);
    gainNode.gain.linearRampToValueAtTime(MIN_VOLUME, now + 120); // Longer decrease duration
